...
metadata:
  annotations:
    config.kubernetess.io/index: 0
    config.kubernetess.io/path: a/b/a_test.yaml
---
c: d # second
metadata:
  annotations:
    config.kubernetess.io/index: 1
    config.kubernetess.io/path: a/b/a_test.yaml
`, string(b))
...
metadata:
  annotations:
    config.kubernetess.io/index: 0
    config.kubernetess.io/path: a/b/b_test.yaml
`, string(b))
}
...
func TestLocalPackageWriter_Write_clearAnnotations(t *testing.T) {
	...
	w := LocalPackageWriter{PackagePath: d, ClearAnnotations: []string{"config.kubernetess.io/mode"}}
	err := w.Write([]*yaml.RNode{node2, node1, node3})
	if !assert.NoError(t, err) {
		assert.FailNow(t, err.Error())
	}
...
metadata:
  annotations:
    config.kubernetess.io/index: a
    config.kubernetess.io/path: "a/b/b_test.yaml" # use a different path, should still collide
`)
	if !assert.NoError(t, err) {
		assert.FailNow(t, err.Error())
	}
	...
	err = w.Write([]*yaml.RNode{node2, node1, node3, node4})
	if assert.Error(t, err) {
		assert.Contains(t, err.Error(), "unable to parse config.kubernetess.io/index")
	}
}

// TestLocalPackageWriter_Write_absPath tests:
// - If config.kubernetess.io/path is absolute, fail
...
metadata:
  annotations:
    config.kubernetess.io/index: a
    config.kubernetess.io/path: "%s/a/b/b_test.yaml" # use a different path, should still collide
`, d))
	if !assert.NoError(t, err) {
		assert.FailNow(t, err.Error())
	}
...
// - If config.kubernetess.io/path is missing, fail
...
`)
	...
	err = w.Write([]*yaml.RNode{node2, node1, node3, node4})
	if assert.Error(t, err) {
		assert.Contains(t, err.Error(), "config.kubernetess.io/path")
	}
}

// TestLocalPackageWriter_Write_missingIndex tests:
// - If config.kubernetess.io/index is missing, fail
...
metadata:
  annotations:
    config.kubernetess.io/path: a/a.yaml
`)
	if !assert.NoError(t, err) {
		assert.FailNow(t, err.Error())
	}
	...
	err = w.Write([]*yaml.RNode{node2, node1, node3, node4})
	if assert.Error(t, err) {
		assert.Contains(t, err.Error(), "config.kubernetess.io/index")
	}
}

// TestLocalPackageWriter_Write_pathIsDir tests:
// - If  config.kubernetess.io/path is a directory, fail
...
`)
	...
	err = w.Write([]*yaml.RNode{node2, node1, node3, node4})
	if assert.Error(t, err) {
		assert.Contains(t, err.Error(), "config.kubernetess.io/path cannot be a directory")
	}
}
