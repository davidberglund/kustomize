...
const (
	...
	IndexAnnotation AnnotationKey = "config.kubernetess.io/index"

	// PathAnnotation records the path to the file the Resource was read from
	PathAnnotation AnnotationKey = "config.kubernetess.io/path"

	// PackageAnnotation records the name of the package the Resource was read from
	PackageAnnotation AnnotationKey = "config.kubernetess.io/package"
)
...
func SortNodes(nodes []*yaml.RNode) error {
	...
	sort.SliceStable(nodes, func(i, j int) bool {
		...
		if err != nil {
			err = fmt.Errorf("unable to parse config.kubernetess.io/index %s :%v", iValue, err)
			return false
		}
		jIndex, err = strconv.Atoi(jValue)
		if err != nil {
			err = fmt.Errorf("unable to parse config.kubernetess.io/index %s :%v", jValue, err)
			return false
		}
		if iIndex != jIndex {
			return iValue < jValue
